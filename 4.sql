...
select 
	...
	case
		...
		when scenario = 'LostRpo' then LostOperation_msk_dts
	...
	case
		...
		when scenario = 'LostRpo' then LostOperation_local_dts
	...
	case
		...
		when scenario = 'LostRpo' then LostOperation_timezone
	...
	case 
		...
		when scenario = 'LostRpo' then LostOperation_border_index
	...
	case
		...
		when scenario = 'LostRpo' then LostOperation_country
		when scenario = 'unknown' then Null 
		else null
...
-- LostOperation
	...
	case 
		when max_lostOperation_flg = 1
		then first_value(local_dts) over (partition by rpobarcode_ccode order by lostOperation_flg DESC, msk_dts DESC)
		else null
	end as LostOperation_msk_dts,
	case 
		when max_lostOperation_flg = 1
		then first_value(local_timezone) over (partition by rpobarcode_ccode order by lostOperation_flg DESC, msk_dts DESC)
		else null
	end as LostOperation_timezone,
	case 
		when max_lostOperation_flg = 1
		then first_value(objectborder_index_ccode) over (partition by rpobarcode_ccode order by lostOperation_flg DESC, msk_dts DESC)
	end as LostOperation_border_index,
	case 
		when max_lostOperation_flg = 1
		then first_value(country_ncode) over (partition by rpobarcode_ccode order by lostOperation_flg DESC, msk_dts DESC)
	end as LostOperation_country,
	isValidExit_msk_dts,
	last_oper_date
from (
select *,
